 ...
 */
#include "aligned_all_gather_asym_double_ring.h"

namespace hccl {
AlignedAllGatherAsymDoubleRing::AlignedAllGatherAsymDoubleRing(
...
// 服务器间allgather的入口函数
HcclResult AlignedAllGatherAsymDoubleRing::RunAsync(const u32 rank, const u32 rankSize, const std::vector<LINK> &links)
{
    // 基本的检查
    ...
    HCCL_INFO("AlignedAllGatherAsymDoubleRing finished: rank[%u] end", rank);
    return HCCL_SUCCESS;
}

HcclResult AlignedAllGatherAsymDoubleRing::CheckParameters(const u32 rank, const u32 rankSize,
                                                          const std::vector<LINK> &links)
{
    ...
    CHK_PRT_RET(subStreams_.size() < 1,
                HCCL_ERROR("[AlignedAllGatherAsymDoubleRing] subStreams size[%u] is less than 1", subStreams_.size()),
                HCCL_E_PARA);
    ...
    CHK_PRT_RET(mainSignals_.size() < 1,
                HCCL_ERROR("[AlignedAllGatherAsymDoubleRing] mainSignals size[%u] is less than 1", mainSignals_.size()),
                HCCL_E_PARA);
    // 判断subSignals数量是否正确
    CHK_PRT_RET(subSignals_.size() < 1,
                HCCL_ERROR("[AlignedAllGatherAsymDoubleRing] subSignals size[%u] is less than 1", subSignals_.size()),
                HCCL_E_PARA);
    // 判断ringsOrder数量是否正确
    for (u32 ringIndex = 0; ringIndex < ringsOrders_.size(); ringIndex++) {
        CHK_PRT_RET(ringsOrders_[ringIndex].size() % rankSize != 0,
                    HCCL_ERROR("[AlignedAllGatherAsymDoubleRing] ringsOrders[%u] size[%u] can not be divided by rank size[%u]",
    ...
    for (u32 ringIndex = 0; ringIndex < userMemOutputSlicesOfDoubleRing_.size(); ringIndex++) {
        CHK_PRT_RET(userMemOutputSlicesOfDoubleRing_[ringIndex].size() % rankSize != 0,
            HCCL_ERROR("[AlignedAllGatherAsymDoubleRing] userMemOutputSlicesOfDoubleRing[%u] size[%u] can not be divided by rank size[%u]",
    ...
    CHK_PRT_RET(mainSliceSize != subSliceSize,
        HCCL_ERROR("[AlignedAllGatherAsymDoubleRing] mainSliceSize[%u] is not equal to subSliceSize[%u].",
            mainSliceSize, subSliceSize),
        HCCL_E_PARA);
    HCCL_INFO("AlignedAllGatherAsymDoubleRing finished to CheckParameters");
    return HCCL_SUCCESS;
}

HcclResult AlignedAllGatherAsymDoubleRing::OneRankMemcpy()
{
...
HcclResult AlignedAllGatherAsymDoubleRing::GetInitializedNeighborLinks(const u32 rank, const u32 rankSize,
                                                                      const std::vector<LINK> &links)
{
    // 收集左邻居信息
    ...
    HCCL_INFO("AlignedAllGatherAsymDoubleRing finished to GetInitializedNeighborLinks");
    return HCCL_SUCCESS;
}

HcclResult AlignedAllGatherAsymDoubleRing::SetSlices(const u32 rank, const u32 rankSize)
{
    for (u32 ringIndex = 0; ringIndex < multRingsSlices_.size(); ringIndex++) {
        ...
        for (u32 i = 0; i < multRingsSlices_[ringIndex].size(); i++) {
            HCCL_DEBUG(
                "[AlignedAllGatherAsymDoubleRing][SetSlices] multRingsSlices_[%u], rank[%u], slices[%u].offset=[%llu], slices[%u].size=[%llu]",
                ringIndex, rank, i, multRingsSlices_[ringIndex][i].offset, i, multRingsSlices_[ringIndex][i].size);
        }
    }
    HCCL_INFO("AlignedAllGatherAsymDoubleRing finished to SetSlices");
    return HCCL_SUCCESS;
}

HcclResult AlignedAllGatherAsymDoubleRing::RunInitStep(const u32 rank, const u32 rankSize)
{
    // for (u32 ringIndex = 0; ringIndex < multRingsSlices_.size(); ringIndex++) {
        ...
        for (u32 sliceIdx = 0; sliceIdx < sliceSize; sliceIdx++) {
            ...
            if (opInfo_->inputAddr != nullptr) {
                // AllGather算子调用AlignedAllGatherAsymDoubleRing场景
                ...
                // AllReduce算子调用AlignedAllGatherAsymDoubleRing场景
                HCCL_DEBUG("Memcpy operation: step[-1] stream[main] src rank[%u] starts to copy(rcv) offset[%llu], "
...
HcclResult AlignedAllGatherAsymDoubleRing::PrepareRunMainStream(u32 ringIndex, Stream &stream,
    LINK &preLink, LINK &nextLink)
{
    HCCL_DEBUG("AlignedAllGatherAsymDoubleRing PrepareRunMainStream start");
    if (ringIndex == 1) {
        stream = stream_;
        preLink = rightLink_;
        nextLink = leftLink_;
    ...
    HCCL_DEBUG("AlignedAllGatherAsymDoubleRing PrepareRunMainStream end");
    return HCCL_SUCCESS;
}

HcclResult AlignedAllGatherAsymDoubleRing::RunAllStreams(const u32 step, const u32 rankSize,
...
HcclResult AlignedAllGatherAsymDoubleRing::RxAsyncMemcpy(const u32 step, const u32 ringIndex, RxMemoryInfo& mem, Stream &stream, LINK &link)
{
    (void)step;
    // PreSync
    if (ringIndex == 1) {
...
HcclResult AlignedAllGatherAsymDoubleRing::LocalMemcpy(const u32 ringIndex,
...
HcclResult AlignedAllGatherAsymDoubleRing::PrepareDeviceMems(
...
HcclResult AlignedAllGatherAsymDoubleRing::RunAllGather(const u32 rank, const u32 rankSize)
{
    HCCL_INFO("AlignedAllGatherAsymDoubleRing starts, the input param rank[%u]", rank);
    if (GetWorkflowMode() != HcclWorkflowMode::HCCL_WORKFLOW_MODE_OPS_KERNEL_INFO_LIB) {
        // 主环主流通知从环主流开始通信
    ...
    CHK_RET(MainWaitSub());
    CHK_RET(ExecutorBase::ExecEmptyTask(inputMem_, outputMem_, stream_, dispatcher_));
    HCCL_INFO("AlignedAllGatherAsymDoubleRing finished to RunAllGather");
    return HCCL_SUCCESS;
}

HcclResult AlignedAllGatherAsymDoubleRing::ExecEmptyTasks()
{
...
// 主流通知从流干活
HcclResult AlignedAllGatherAsymDoubleRing::MainRecordSub()
{
...
// 从流等待主流
HcclResult AlignedAllGatherAsymDoubleRing::SubWaitMain()
{
...
// 主流等待从流
HcclResult AlignedAllGatherAsymDoubleRing::MainWaitSub()
{
...
// 从流告诉主流活干完了
HcclResult AlignedAllGatherAsymDoubleRing::SubRecordMain()
{

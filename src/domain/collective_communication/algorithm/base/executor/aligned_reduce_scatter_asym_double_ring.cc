...
#include "aligned_reduce_scatter_asym_double_ring.h"

namespace hccl {
AlignedReduceScatterAsymDoubleRing::AlignedReduceScatterAsymDoubleRing(
...
HcclResult AlignedReduceScatterAsymDoubleRing::RunAsync(const u32 rank, const u32 rankSize,
                                                       const std::vector<LINK> &links)
{
    ...
    HCCL_INFO("AlignedReduceScatterAsymDoubleRing finished: rank[%u] end", rank);
    return HCCL_SUCCESS;
}

HcclResult AlignedReduceScatterAsymDoubleRing::CheckParameters(const u32 rank, const u32 rankSize,
                                                              const std::vector<LINK> &links)
{
    ...
    CHK_PRT_RET(
        subStreams_.size() < 1,
        HCCL_ERROR("[AlignedReduceScatterAsymDoubleRing] subStreams size[%u] is less than 1", subStreams_.size()),
        HCCL_E_PARA);
    ...
    CHK_PRT_RET(
        mainSignals_.size() < 1,
        HCCL_ERROR("[AlignedReduceScatterAsymDoubleRing] mainSignals size[%u] is less than 1", mainSignals_.size()),
        HCCL_E_PARA);
    // 判断subSignals数量是否正确
    CHK_PRT_RET(
        subSignals_.size() < 1,
        HCCL_ERROR("[AlignedReduceScatterAsymDoubleRing] subSignals size[%u] is less than 1", subSignals_.size()),
        HCCL_E_PARA);
    // 判断ringsOrder数量是否正确
    for (u32 ringIndex = 0; ringIndex < ringsOrders_.size(); ringIndex++) {
        CHK_PRT_RET(ringsOrders_[ringIndex].size() != rankSize,
            HCCL_ERROR("[AlignedReduceScatterAsymDoubleRing] ringsOrders[%u] size[%u] is not equal to rank size[%u]",
    ...
    for (u32 ringIndex = 0; ringIndex < userMemInputSlicesOfDoubleRing_.size(); ringIndex++) {
        CHK_PRT_RET(userMemInputSlicesOfDoubleRing_[ringIndex].size() % rankSize != 0,
            HCCL_ERROR("[AlignedReduceScatterAsymDoubleRing] userMemInputSlicesOfDoubleRing[%u] size[%u] can not divided by size[%u]",
    ...
    CHK_PRT_RET(mainSliceSize != subSliceSize,
        HCCL_ERROR("[AlignedReduceScatterAsymDoubleRing] mainSliceSize[%u] is not equal to subSliceSize[%u].",
            mainSliceSize, subSliceSize),
        HCCL_E_PARA);
    HCCL_INFO("AlignedReduceScatterAsymDoubleRing finished to CheckParameters");
    return HCCL_SUCCESS;
}

HcclResult AlignedReduceScatterAsymDoubleRing::OneRankMemcpy()
{
...
HcclResult AlignedReduceScatterAsymDoubleRing::InitSenderReducer()
{
    // 创建reducer & sender
    ...
    HCCL_INFO("AlignedReduceScatterAsymDoubleRing finished to InitSenderReducer");
    return HCCL_SUCCESS;
}

HcclResult AlignedReduceScatterAsymDoubleRing::GetInitializedNeighborLinks(const u32 rank, const u32 rankSize,
                                                                          const std::vector<LINK> &links)
{
    // 收集左邻居信息
    ...
    HCCL_INFO("AlignedReduceScatterAsymDoubleRing finished to GetInitializedNeighborLinks");
    return HCCL_SUCCESS;
}

HcclResult AlignedReduceScatterAsymDoubleRing::SetSlices(const u32 rank, const u32 rankSize)
{
    for (u32 ringIndex = 0; ringIndex < multRingsSlices_.size(); ringIndex++) {
        ...
        for (u32 i = 0; i < multRingsSlices_[ringIndex].size(); i++) {
            HCCL_DEBUG(
                "[AlignedReduceScatterAsymDoubleRing][SetSlices] multRingsSlices_[%u], rank[%u], "
    ...
    HCCL_INFO("AlignedReduceScatterAsymDoubleRing finished to SetSlices");
    return HCCL_SUCCESS;
}

HcclResult AlignedReduceScatterAsymDoubleRing::PrepareInitSlices(const u32 rankSize,
...
HcclResult AlignedReduceScatterAsymDoubleRing::MemcpyInitSlicesOnMainStreams(
...
HcclResult AlignedReduceScatterAsymDoubleRing::RunInitStep(const u32 rank, const u32 rankSize)
{
...
HcclResult AlignedReduceScatterAsymDoubleRing::PrepareRunMainStream(u32 ringIndex, Stream &stream,
    LINK &preLink, LINK &nextLink)
{
    HCCL_DEBUG("AlignedReduceScatterAsymDoubleRing PrepareRunMainStream start");
    if (ringIndex == 1) {
        stream = stream_;
        preLink = rightLink_;
        nextLink = leftLink_;
    ...
    HCCL_DEBUG("AlignedReduceScatterAsymDoubleRing PrepareRunMainStream end");
    return HCCL_SUCCESS;
}

HcclResult AlignedReduceScatterAsymDoubleRing::PreSync(const u32 ringIndex)
{
    if (ringIndex == 1) {
        CHK_RET(MainWaitSub());
        CHK_RET(ExecutorBase::ExecEmptyTask(inputMem_, outputMem_, stream_, dispatcher_));
    ...
    std::vector<DeviceMem> &localSrcMems, std::vector<DeviceMem> &localDstMems)
{
    ...
    for (u32 sliceIdx = 0; sliceIdx < sliceSize; sliceIdx++) {
        ...
        u32 DMA_REDUCE_ASYM_OFFSET = rankSize / 2 + 1;
        DeviceMem dst;
        if (step == rankSize - DMA_REDUCE_ASYM_OFFSET && opInfo_->outputAddr != nullptr) {
...
HcclResult AlignedReduceScatterAsymDoubleRing::RxAsyncMemcpy(
...
HcclResult AlignedReduceScatterAsymDoubleRing::ReducerRun(const u32 ringIndex, const HcclDispatcher dispatcher,
    const LINK &link,
...
HcclResult AlignedReduceScatterAsymDoubleRing::RunMainStream(
...
HcclResult AlignedReduceScatterAsymDoubleRing::LocalMemcpy(const u32 step, const u32 rankSize, const u32 ringIndex,
...
HcclResult AlignedReduceScatterAsymDoubleRing::RunSubStream(
...
HcclResult AlignedReduceScatterAsymDoubleRing::RunAllStreams(const u32 step, const u32 rankSize,
...
HcclResult AlignedReduceScatterAsymDoubleRing::RunReduceScatter(const u32 rank, const u32 rankSize)
{
    HCCL_INFO("AlignedReduceScatterAsymDoubleRing starts, the input param rank[%u]", rank);
    CHK_RET(ExecutorBase::ExecEmptyTask(inputMem_, outputMem_, stream_, dispatcher_));
    // 主环主流通知从环主流开始通信
    CHK_RET(MainRecordSub());
    // 从环主流等待主环主流通知
    ...
    // 例如rank[0,1,2,3]中，rank0的rxSliceIdx = 1，txSliceIdx = 2, subSliceIdx = 3
    // 例如rank[0,1,2,3]中，rank0的rxSliceIdx = 3，txSliceIdx = 2, subSliceIdx = 3
    ...
    CHK_RET(ExecutorBase::ExecEmptyTask(inputMem_, outputMem_, stream_, dispatcher_));
    CHK_RET(ExecEmptyTasks());
    HCCL_INFO("AlignedReduceScatterAsymDoubleRing finished to RunReduceScatter");
    return HCCL_SUCCESS;
}

HcclResult AlignedReduceScatterAsymDoubleRing::GetActiveSubstreamNum(u32 &activeSubstreamNum)
{
    activeSubstreamNum = subStreams_.size();
    if (GetWorkflowMode() != HcclWorkflowMode::HCCL_WORKFLOW_MODE_OPS_KERNEL_INFO_LIB &&
        retryEnable_) {
        if (subStreams_.size() <= 2) {
            HCCL_ERROR("[AlignedReduceScatterAsymDoubleRing][GetActiveSubstreamNum]subStreams_.size()[%zu] <= 2",
                subStreams_.size());
            return HCCL_E_PARA;
        }
...
HcclResult AlignedReduceScatterAsymDoubleRing::ExecEmptyTasks()
{
...
// 主流通知从流干活
HcclResult AlignedReduceScatterAsymDoubleRing::MainRecordSub()
{
    u32 activeSubstreamNum = 0;
    CHK_RET(GetActiveSubstreamNum(activeSubstreamNum));
    for (u32 signalIndex = 0; signalIndex < activeSubstreamNum; signalIndex++) {
...
// 从流等待主流
HcclResult AlignedReduceScatterAsymDoubleRing::SubWaitMain()
{
    u32 activeSubstreamNum = 0;
    CHK_RET(GetActiveSubstreamNum(activeSubstreamNum));
...
// 主流等待从流
HcclResult AlignedReduceScatterAsymDoubleRing::MainWaitSub()
{
    u32 activeSubstreamNum = 0;
    CHK_RET(GetActiveSubstreamNum(activeSubstreamNum));
    for (u32 signalIndex = 0; signalIndex < activeSubstreamNum; signalIndex++) {
...
// 从流告诉主流活干完了
HcclResult AlignedReduceScatterAsymDoubleRing::SubRecordMain()
{
    u32 activeSubstreamNum = 0;
    CHK_RET(GetActiveSubstreamNum(activeSubstreamNum));
    for (u32 streamIndex = 0; streamIndex < activeSubstreamNum; streamIndex++) {

 ...
 */
#include "aligned_all_gather_asym_all_double_ring.h"

// allreduce 打平 allgather强制从CCL In 读取数据

namespace hccl {
AlignedAllGatherAsymAllDoubleRing::AlignedAllGatherAsymAllDoubleRing(
...
// 服务器间allgather的入口函数
HcclResult AlignedAllGatherAsymAllDoubleRing::RunAsync(const u32 rank, const u32 rankSize, const std::vector<LINK> &links)
{
    // 基本的检查
    ...
    HCCL_INFO("AlignedAllGatherAsymAllDoubleRing finished: rank[%u] end", rank);
    return HCCL_SUCCESS;
}

HcclResult AlignedAllGatherAsymAllDoubleRing::CheckParameters(const u32 rank, const u32 rankSize,
                                                          const std::vector<LINK> &links)
{
    ...
    CHK_PRT_RET(subStreams_.size() < 1,
                HCCL_ERROR("[AlignedAllGatherAsymAllDoubleRing] subStreams size[%u] is less than 1", subStreams_.size()),
                HCCL_E_PARA);
    ...
    CHK_PRT_RET(mainSignals_.size() < 1,
                HCCL_ERROR("[AlignedAllGatherAsymAllDoubleRing] mainSignals size[%u] is less than 1", mainSignals_.size()),
                HCCL_E_PARA);
    // 判断subSignals数量是否正确
    CHK_PRT_RET(subSignals_.size() < 1,
                HCCL_ERROR("[AlignedAllGatherAsymAllDoubleRing] subSignals size[%u] is less than 1", subSignals_.size()),
                HCCL_E_PARA);
    // 判断ringsOrder数量是否正确
    for (u32 ringIndex = 0; ringIndex < ringsOrders_.size(); ringIndex++) {
        CHK_PRT_RET(ringsOrders_[ringIndex].size() % rankSize != 0,
                    HCCL_ERROR("[AlignedAllGatherAsymAllDoubleRing] ringsOrders[%u] size[%u] can not be divided by rank size[%u]",
    ...
    for (u32 ringIndex = 0; ringIndex < userMemOutputSlicesOfDoubleRing_.size(); ringIndex++) {
        CHK_PRT_RET(userMemOutputSlicesOfDoubleRing_[ringIndex].size() % rankSize != 0,
            HCCL_ERROR("[AlignedAllGatherAsymAllDoubleRing] userMemOutputSlicesOfDoubleRing[%u] size[%u] can not be divided by rank size[%u]",
    ...
    CHK_PRT_RET(mainSliceSize != subSliceSize,
        HCCL_ERROR("[AlignedAllGatherAsymAllDoubleRing] mainSliceSize[%u] is not equal to subSliceSize[%u].",
            mainSliceSize, subSliceSize),
        HCCL_E_PARA);
    HCCL_INFO("AlignedAllGatherAsymAllDoubleRing finished to CheckParameters");
    return HCCL_SUCCESS;
}

HcclResult AlignedAllGatherAsymAllDoubleRing::OneRankMemcpy()
{
...
HcclResult AlignedAllGatherAsymAllDoubleRing::GetInitializedNeighborLinks(const u32 rank, const u32 rankSize,
                                                                      const std::vector<LINK> &links)
{
    // 收集左邻居信息
    ...
    HCCL_INFO("AlignedAllGatherAsymAllDoubleRing finished to GetInitializedNeighborLinks");
    return HCCL_SUCCESS;
}

HcclResult AlignedAllGatherAsymAllDoubleRing::SetSlices(const u32 rank, const u32 rankSize)
{
    for (u32 ringIndex = 0; ringIndex < multRingsSlices_.size(); ringIndex++) {
        ...
        for (u32 i = 0; i < multRingsSlices_[ringIndex].size(); i++) {
            HCCL_DEBUG(
                "[AlignedAllGatherAsymAllDoubleRing][SetSlices] multRingsSlices_[%u], rank[%u], slices[%u].offset=[%llu], slices[%u].size=[%llu]",
                ringIndex, rank, i, multRingsSlices_[ringIndex][i].offset, i, multRingsSlices_[ringIndex][i].size);
        }
    }
    HCCL_INFO("AlignedAllGatherAsymAllDoubleRing finished to SetSlices");
    return HCCL_SUCCESS;
}

HcclResult AlignedAllGatherAsymAllDoubleRing::RunInitStep(const u32 rank, const u32 rankSize)
{
    // for (u32 ringIndex = 0; ringIndex < multRingsSlices_.size(); ringIndex++) {
        ...
        for (u32 sliceIdx = 0; sliceIdx < sliceSize; sliceIdx++) {
            ...
            if (opInfo_->inputAddr != nullptr) {
                // AllGather算子调用AlignedAllGatherAsymAllDoubleRing场景
                ...
                // AllReduce算子调用AlignedAllGatherAsymAllDoubleRing场景
                HCCL_DEBUG("Memcpy operation: step[-1] stream[main] src rank[%u] starts to copy(rcv) offset[%llu], "
...
HcclResult AlignedAllGatherAsymAllDoubleRing::PrepareRunMainStream(u32 ringIndex, Stream &stream,
    LINK &preLink, LINK &nextLink)
{
    HCCL_DEBUG("AlignedAllGatherAsymAllDoubleRing PrepareRunMainStream start");
    if (ringIndex == 1) {
        stream = stream_;
        preLink = rightLink_;
        nextLink = leftLink_;
    ...
    HCCL_DEBUG("AlignedAllGatherAsymAllDoubleRing PrepareRunMainStream end");
    return HCCL_SUCCESS;
}

HcclResult AlignedAllGatherAsymAllDoubleRing::RunAllStreams(const u32 step, const u32 rankSize,
...
HcclResult AlignedAllGatherAsymAllDoubleRing::RxAsyncMemcpy(const u32 step, const u32 ringIndex, RxMemoryInfo& mem, Stream &stream, LINK &link)
{
    (void)step;
    // PreSync
    if (ringIndex == 1) {
...
HcclResult AlignedAllGatherAsymAllDoubleRing::LocalMemcpy(const u32 ringIndex,
...
HcclResult AlignedAllGatherAsymAllDoubleRing::PrepareDeviceMems(
...
HcclResult AlignedAllGatherAsymAllDoubleRing::RunAllGather(const u32 rank, const u32 rankSize)
{
    HCCL_INFO("AlignedAllGatherAsymAllDoubleRing starts, the input param rank[%u]", rank);
    if (GetWorkflowMode() != HcclWorkflowMode::HCCL_WORKFLOW_MODE_OPS_KERNEL_INFO_LIB) {
        // 主环主流通知从环主流开始通信
    ...
    CHK_RET(MainWaitSub());
    CHK_RET(ExecutorBase::ExecEmptyTask(inputMem_, outputMem_, stream_, dispatcher_));
    HCCL_INFO("AlignedAllGatherAsymAllDoubleRing finished to RunAllGather");
    return HCCL_SUCCESS;
}

HcclResult AlignedAllGatherAsymAllDoubleRing::ExecEmptyTasks()
{
...
// 主流通知从流干活
HcclResult AlignedAllGatherAsymAllDoubleRing::MainRecordSub()
{
...
// 从流等待主流
HcclResult AlignedAllGatherAsymAllDoubleRing::SubWaitMain()
{
...
// 主流等待从流
HcclResult AlignedAllGatherAsymAllDoubleRing::MainWaitSub()
{
...
// 从流告诉主流活干完了
HcclResult AlignedAllGatherAsymAllDoubleRing::SubRecordMain()
{

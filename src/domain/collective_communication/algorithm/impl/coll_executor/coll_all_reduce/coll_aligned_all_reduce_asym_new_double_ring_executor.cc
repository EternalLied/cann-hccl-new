...
#include "coll_aligned_all_reduce_asym_new_double_ring_executor.h"

namespace hccl {

CollAlignedAllReduceAsymNewDoubleRingExecutor::CollAlignedAllReduceAsymNewDoubleRingExecutor(
...
HcclResult CollAlignedAllReduceAsymNewDoubleRingExecutor::DoubleRingReduceScatter(const std::string &tag,
...
{
    (void)tag;
    HCCL_INFO("[CollAlignedAllReduceAsymNewDoubleRingExecutor][DoubleRingReduceScatter] DoubleRingReduceScatter starts");
    ...
    executor.reset(new (std::nothrow) AlignedReduceScatterAsymDoubleRing(dispatcher_,
    ...
    CHK_PRT_RET(ret != HCCL_SUCCESS,
        HCCL_ERROR("[CollAlignedAllReduceAsymNewDoubleRingExecutor][DoubleRingReduceScatter] Double ring "
...
HcclResult CollAlignedAllReduceAsymNewDoubleRingExecutor::DoubleRingAllGather(
...
{
    (void)tag;
    HCCL_INFO("[CollAlignedAllReduceAsymNewDoubleRingExecutor][DoubleRingAllGather] DoubleRingAllGather starts");
    HcclResult ret = HCCL_SUCCESS;
    u32 ringNum = multRingsSliceZero.size();
    CHK_RET(CheckCommSize(COMM_LEVEL0, ringNum));
    // 拿到ring环映射关系
    ...
    CHK_PRT_RET(ret != HCCL_SUCCESS,
        HCCL_ERROR("[CollAlignedAllReduceAsymNewDoubleRingExecutor][DoubleRingAllGather]Double ring "
...
HcclResult CollAlignedAllReduceAsymNewDoubleRingExecutor::RunIntraSeverReduceScatter(
    const std::string &tag, DeviceMem &inputMem, DeviceMem &outputMem,
...
HcclResult CollAlignedAllReduceAsymNewDoubleRingExecutor::RunIntraSeverAllGather(
    const std::string &tag, DeviceMem &inputMem, DeviceMem &outputMem,
    const u64 count, const HcclDataType &dataType, const std::vector<std::vector<Slice>> &multRingsSliceZero,
...
REGISTER_EXEC("AlignedAllReduceAsymNewDoubleRingExecutor", AlignedAllReduceAsymNewDoubleRing,
    CollAlignedAllReduceAsymNewDoubleRingExecutor);

 ...
 */
#include "coll_aligned_all_gather_asym_new_double_ring_executor.h"
#include "hccl_types.h"

namespace hccl {
CollAlignedAllGatherAsymNewDoubleRingExecutor::CollAlignedAllGatherAsymNewDoubleRingExecutor(
...
HcclResult CollAlignedAllGatherAsymNewDoubleRingExecutor::DoubleRingAllGather(
...
{
    (void)tag;
    HCCL_INFO("[CollAlignedAllGatherAsymNewDoubleRingExecutor][DoubleRingAllGather] DoubleRingAllGather starts");
    ...
    CHK_PRT_RET(ret != HCCL_SUCCESS,
        HCCL_ERROR("[CollAlignedAllGatherAsymNewDoubleRingExecutor][DoubleRingAllGather]Double ring "
...
HcclResult CollAlignedAllGatherAsymNewDoubleRingExecutor::RunIntraSeverAllGather(
    const std::string &tag, DeviceMem &inputMem, DeviceMem &outputMem,
    const u64 count, const HcclDataType &dataType, const std::vector<std::vector<Slice>> &multRingsSliceZero,
...
REGISTER_EXEC("AlignedAllGatherAsymNewDoubleRingExecutor", AlignedAllGatherAsymNewDoubleRing,
    CollAlignedAllGatherAsymNewDoubleRingExecutor);
